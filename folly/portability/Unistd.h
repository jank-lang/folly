...
#ifdef __MINGW64__
// MSYS2 provides POSIX APIs similar to Folly's unistd portability layer.
// Avoid importing Folly's implementations globally to prevent conflicts.
// Jank only uses Folly's unistd functionality indirectly through
// folly::Synchronized. The function below is required by Folly but is not
// provided by MSYS2.
using folly::portability::unistd::sysconf;
#else
FOLLY_PUSH_WARNING
